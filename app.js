...
async function handleGenerate() {
  ...
  if (!apiKey.startsWith('AIza'))
    return showError('Invalid API key format. Gemini keys start with "AIza..."');
  if (!dayDesc) return showError('Please describe your day so I can personalize your meal plan.');
...
- **Budget:** ${budget}
- **Cuisine Preference:** ${cuisine === 'any' ? 'Any / open to suggestions' : cuisine}
...
async function callGemini(apiKey, prompt) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 400) throw new Error('Invalid API key or request. Check your Gemini API key.');
...
function parseGeminiResponse(response) {
  try {
    ...
    if (!parsed.meals || !parsed.groceryList || !parsed.substitutions || !parsed.budget) {
...
function renderBudget(budget) {
  ...
  const total = typeof budget.estimatedTotal === 'number'
...
function renderGroceryList(groceryList) {
  ...
  groceryList.forEach((item, i) => {
    ...
    li.innerHTML = `
      <input type="checkbox" id="chk-${i}" aria-label="Mark ${item.item} as bought" />
...
function setLoading(on) {
  ...
  btnText.hidden   = on;
  btnLoader.hidden = !on;
  if (on) regenBtn.disabled = true;
  else     regenBtn.disabled = false;
}
...
function buildFullPlanText(plan) {
  ...
  const lines = [
    ...
    '══════════════════════════════════',
    '🛒 GROCERY LIST',
    ...plan.groceryList.map(g => `• ${g.item} — ${g.quantity} (≈ ₹${g.estimatedCost})`),
    '',
    '══════════════════════════════════',
    '🔄 SUBSTITUTIONS',
    ...plan.substitutions.map(s => `• ${s.original} → ${s.substitute}: ${s.reason}`),
    '',
    '══════════════════════════════════',
